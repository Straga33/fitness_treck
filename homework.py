...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        run_spent_calories: float = ((coff_calorie_1 * self.get_mean_speed() - coff_calorie_2)
                                     * self.weight / self.M_IN_KM * self.duration * self.HOUR_MIN)
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        swl_spent_calories: float = ((coff_calorie_1 * self.weight + (self.get_mean_speed()**2 // self.height)
                                     * coff_calorie_2 * self.weight) * self.duration * self.HOUR_MIN)
...
class Swimming(Training):
    ...
    def __init__(self, action, duration, weight, length_pool: int, count_pool: int) -> None:
        super().__init__(action, duration, weight)
        self.length_pool = length_pool
        self.count_pool = count_pool

    def get_mean_speed(self) -> float:
        swm_mean_speed: float = (self.length_pool * self.count_pool / self.M_IN_KM / self.duration)
